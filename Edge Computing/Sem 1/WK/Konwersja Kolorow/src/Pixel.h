...
struct Pixel
{
    ...
    Pixel& operator=(unsigned char rgb[3])
    {
        for (int i = 0; i != 3; ++i)
            this->rgb[i] = rgb[i];
        return *this;
    ...
    {
        rgb[0] = min(static_cast<int>(rgb[0]), pixelMax);
        rgb[1] = min(static_cast<int>(rgb[1]), pixelMax);
        rgb[2] = min(static_cast<int>(rgb[2]), pixelMax);
    }
...
struct GreyscalePixel
{
    ...
    GreyscalePixel& operator=(unsigned char a)
    {
        this->a = a;
        return *this;
    }
    ...
    void setPixelMax(int pixelMax)
    {
        a = min(static_cast<int>(a), pixelMax);
    }
